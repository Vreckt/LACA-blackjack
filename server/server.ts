...
import { environnement } from './src/environments/environnement';
...
server.listen(environnement.port, () => console.log(`Version: 0.0.1, Server listening at port ${environnement.port}`));
...
io.on(SocketEnum.CONNECTION, (socket: any) => {
    ...
    socket.on(SocketEnum.CREATENEWGAME, (data: any) => {
    ...
    socket.on(SocketEnum.JOINGAME, (data: any) => {
    ...
    socket.on(SocketEnum.KICKPLAYER, (data: any) => {
